...
@endif
    ...
    <div class="page-header float-left trn-list">
        <a href="{{ route('admin.menu-category.create') }}">
...
<table id="example" class="table table-hover responsive nowrap" style="width:100%">
  ...
  <tbody>
      @foreach($menuCategories as $key => $categories)
          <tr>
              <td>{{$categories->name}}</td>
              <td>{{$categories->status}}</td>
              <td>
              <a href="{{ route('admin.menu-category.edit', $categories->id) }}"
                title="Edit Item Categories" class="cmn-btn btn_blue"><button class=""><i class="fa fa-edit"></i></button></a>
              
                <form method="POST" id="delete-form-{{ $categories->id }}" action="{{ route('admin.menu-category.destroy', $categories->id) }}" style="display: none;">
                    {{ csrf_field() }}
                    {{ method_field('delete') }}
                </form>
