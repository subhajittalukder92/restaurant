...
<table id="example" class="table table-hover responsive nowrap" style="width:100%">
  <thead>
    <tr>
      <th scope="col">Coin( No. of Coin)</th>
      <th scope="col">Amount</th>
      <th scope="col" >Action </th>
    </tr>
  </thead>
  <tbody>
      @foreach($rewards as $key => $itemData)
          <tr class="custom-table-row" data-url="{{ route('admin.reward.show', $itemData->id) }}">
             <td>{{$itemData->coins}}</td>
             <td>{{$itemData->amount}}</td>
             <td>
             <a href="{{ route('admin.reward.edit', $itemData->id) }}"
              title="Edit Trainer" class="cmn-btn btn_blue"><button class=""><i class="fa fa-edit"></i></button></a>
             
              <form method="POST" id="delete-form-{{ $itemData->id }}" action="{{ route('admin.reward.destroy', $itemData->id) }}" style="display: none;">
                  {{ csrf_field() }}
                  {{ method_field('delete') }}
              </form>
